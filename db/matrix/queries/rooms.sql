...
SELECT ra.room_id, rm.members, ev.origin_server_ts, ev.sender as owner, 
    rooms.is_public,
	jsonb_build_object('name', rs.name ,'type', rs.type, 'is_profile', rs.is_profile, 'topic', rs.topic, 'avatar', rs.avatar, 'header', rs.header, 'restrictions', rs.restrictions, 'pinned_events', rs.pinned_events, 'topics', room_topics.topics) as state,
...
LEFT JOIN (
    	SELECT rst.room_id, rst.name, rst.type, rst.topic, rst.avatar, rst.header, rst.restrictions, sc.child_room_alias, sc.parent_room_id, events.origin_server_ts, rstm.topics, rst.pinned_events
	FROM room_state rst
...
GROUP BY ra.room_id, rm.members, ev.origin_server_ts, ev.sender, rooms.is_public, rs.name, rs.is_profile, rs.type, rs.topic, rs.avatar, rs.header, rs.pinned_events, rs.restrictions, room_topics.topics, ms.membership;
